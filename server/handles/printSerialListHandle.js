 ...
 */
const objectModel = require("../data/serialPrintModel");
...
async function searchByClient(_client) {
    return new Promise(resolve => {
        if (_client && _client.client) {
            objectModel.aggregate(
                [{
                    $match: { client: _client.client }
                },
                {
                    $group: {
                        _id: "$sku",
...
module.exports = {
    save, create, getList, getOne, createUniqueStringList, searchByClient
};