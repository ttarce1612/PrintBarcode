...
export default function Inbound() {
  ...
  dispatcher.register('inbound_add_new', (payload) => {
    if (payload && payload.data) {
      if (payload.action === 'CREATE') {
        setOpen(true);
        grantData(null);
      } else {
        inboundService.getOne(payload.data._id)
          .then((result) => {
            if (result.data) {
  ...
  const handleSave = () => {
    ...
    if (modelData.action === 'UPDATE') {
      inboundService.update(modelData);
    } else {
      inboundService.create(modelData);
    }
  ...
  React.useEffect(() => {
    return () => {
      dispatcher.destroy('inbound_add_new');
    };
  }, []);
