...
module.exports = {
    PrintSerial: (postData) => {
        Request.post("/api/serialprint/print", postData)
            .then((res) => {
                if (res.status == 200 && res.data && res.data.Data) {
                    let _data = res.data.Data
                    toast.info("Generate completed, Successed: " + _data.sucess + " , Failed: " + _data.failed);
                    setTimeout(function(){
                        toast.info("Start printing");
                    }, 1000);
                    printProcess.PrintSerial(_data)
                }
            })
    },
